...
def confirm_request():
    ...
    get_queue().enqueue(
        ...
        user=current_user._get_current_object(),
        confirm_link=confirm_link)
    #print("job id = {}".format(dir(job.id)))
    user=current_user._get_current_object()
    send_email(recipient=current_user.email, subject='Confirm Your Account', template='account/email/confirm', user=user)
